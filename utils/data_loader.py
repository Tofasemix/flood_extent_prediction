...
class CycloneFloodDataset(Dataset):
    def __init__(
        ...
        self.tabular_cols = ['pressure', 'vmax', 'wind']
        self.raw_tabular_data = self.data_frame[self.tabular_cols].values

        # Normalization protocol:
        # - Legacy behavior: if no statistics are supplied, fit mean/std
        #   from the CSV passed to this dataset.
        # - Research protocol: validation/test datasets receive statistics
        #   fitted ONLY from the training split.
        if (tabular_mean is None) != (tabular_std is None):
            raise ValueError(
                "tabular_mean and tabular_std must either both be provided "
                "or both be None."
            )

        if tabular_mean is None:
            # Preserve the original normalization behavior exactly.
            self.tab_mean = self.raw_tabular_data.mean(axis=0)
            self.tab_std = self.raw_tabular_data.std(axis=0)
            self.normalization_source = "fitted_from_current_csv"
        else:
            self.tab_mean = np.asarray(tabular_mean).copy()
            self.tab_std = np.asarray(tabular_std).copy()
            self.normalization_source = "provided_external_stats"

            expected_shape = (len(self.tabular_cols),)

            if self.tab_mean.shape != expected_shape:
                raise ValueError(
                    f"tabular_mean must have shape {expected_shape}, "
                    f"got {self.tab_mean.shape}."
                )

            if self.tab_std.shape != expected_shape:
                raise ValueError(
                    f"tabular_std must have shape {expected_shape}, "
                    f"got {self.tab_std.shape}."
                )

        # Normalize the tabular data immediately to prevent dominating weights.
        self.tabular_data = (
            self.raw_tabular_data - self.tab_mean
        ) / (self.tab_std + 1e-8)

    def get_tabular_stats(self):
        """Return copies of the normalization statistics used by this dataset."""
        return self.tab_mean.copy(), self.tab_std.copy()
    ...
    def __getitem__(self, idx):
        ...
        tp36_path = os.path.join(
            self.img_dir,
            'PRECIPITATION',
            f'TP36_{cyclone_id}.png',
        )
        slope_path = os.path.join(
            self.img_dir,
            'SLOPE',
            f'SLOPE_{cyclone_id}.png',
        )
        flood_path = os.path.join(
            self.img_dir,
            'FLOOD',
            f'FLOOD_{cyclone_id}.png',
        )

        # 3. Load Images
        # cv2.IMREAD_GRAYSCALE ensures they load as 2D 512x512 arrays.
        ...
        # This randomly flips/rotates both the inputs and the target mask
